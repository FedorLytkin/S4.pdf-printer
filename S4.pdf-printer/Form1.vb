    ...
    Sub create_newpdf(PDFfileName As String, OTD_REGNUM As String, OTD_reg As String, FirstDoc As Boolean)
        ...
        Try
            ...
            For i As Integer = 1 To reader.NumberOfPages
                ...
                'штамп
                If Convert.ToBoolean(Option_app.stamp_create) Then
                    Try
                        ...
                        logo.SetAbsolutePosition(size.Right - 220 - 20 * delta, 175 + 15 * delta)
                ...
                cb.BeginText()
                cb.SetFontAndSize(bf, 10 + delta)
                cb.ShowTextAligned(0, S4Username, 30 + 10 * delta, 5 + delta, 0)
                cb.EndText()

                'дата печати
                cb.BeginText()
                TextOnPDF = Now.ToShortDateString & " " & Now.ToShortTimeString
                cb.ShowTextAligned(0, TextOnPDF, 200 + 15 * delta, 5 + delta, 0)
                cb.EndText()
                ...
                cb.ShowTextAligned(0, OTD_REGNUM, 53 + delta, 25 + 10 * delta, 90)
                cb.EndText()

                'дата регистрации
                cb.BeginText()
                cb.ShowTextAligned(0, OTD_reg, 53 + delta, 95 + 20 * delta, 90)
                cb.EndText()
